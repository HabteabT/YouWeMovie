...
namespace YouWeMovie.Areas.Identity.Pages.Account
...

{
    public class RegisterModel : PageModel
    {
        ...
        public class InputModel
        {
            ...
            [CustomValidation(typeof(RegisterModel), nameof(MaxFileSize))]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // here is email and user name 

                await _userStore.SetUserNameAsync(user, Input.Email, CancellationToken.None);
                await _emailStore.SetEmailAsync(user, Input.Email, CancellationToken.None); 
                
                
                // we have to write here what we want 
                user.NickName = Input.NickName;
                user.Age = Input.Age;
                user.RegTime = DateTime.UtcNow;
                
                if (Input.Pic is { Length: > 0 })
                {
                    using var memoryStream = new MemoryStream();
                    await Input.Pic.CopyToAsync(memoryStream);
